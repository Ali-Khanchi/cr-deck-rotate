...
const mapCards = (): { [key: string]: CardItem } => {
    return cardEndpoint.items
        .filter((e: CardItem) => e.iconUrls.evolutionMedium)
        .map((e: CardItem) => {
            const c = structuredClone(e);
            c.name = `Evolved ${c.name}`;
            c.iconUrls.medium = c.iconUrls.evolutionMedium || "";
            return c;
        })
